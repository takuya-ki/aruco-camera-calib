...
save_board_dirpath = osp.join(
    osp.dirname(__file__), "../data/board/")
board_filename = "sample_board.png"
dictionary = aruco.getPredefinedDictionary(
    aruco.DICT_5X5_100)
squareL = 0.028
markerL = 0.024
pixels_per_mm = 10 # checker board image
A4size = (210, 297)
...
# unit: mm
squareNumX, boardSizeX = max_within_upper(squareL*1000, A4size[0]- lr*2 )
squareNumY, boardSizeY = max_within_upper(squareL*1000, A4size[1]- tb*2 )
...
def create_ChArUco_board_in_A4size(board_name, outcsv=True):
    ...
    if not osp.isdir(save_board_dirpath):
        os.mkdir(save_board_dirpath)
    
    board_filepath = osp.join(save_board_dirpath, board_name)
    ...
    if outcsv:
        ...
        filename = osp.splitext(board_name)[0]
        output_board_configs_to_csv(
            osp.join(save_board_dirpath, 
                         filename+'.csv'))


if __name__ == '__main__':
    create_ChArUco_board_in_A4size(board_filename)
